...
export const addProduct = async (req, res) => {
    try {
        ...
        const { type_name, ...otherProductData } = req.body;

        
        const typeResult = await product_types.get({ type_name });
        ...
        const result = await products.add(productData);
        if (!result.success) {
            return res.status(400).json({ message: result.message });
        }

        const { id, name, offering_type, is_available, price } = result.createProduct;
        res.status(201).json({ id, name, type_id, offering_type, offered_by: DEFAULT_OFFERED_BY, is_available, price });
    } catch (error) {
